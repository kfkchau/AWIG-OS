...
|---|---|---|---|---|---|---|---|
| 1 | laptop / desktop | Linux (macOS, Windows: same shape, different host) | FULL BODY | HIGH on Linux, LOW elsewhere | now / AIR for other hosts | HIGH | this estate IS one; the M3 and W4 worlds are laptop records |
...
| 5 | phone / tablet | Android, iOS — sandboxed apps, store gate | FULL BODY inside one app's sandbox; ACTOR beyond it | LOW | AIR (grammar as a library) · C7 for the OS | MEDIUM | the sandbox is a body the host vouches for; the store gate is an outside decision record; no root access is not a blocker, it is the guest pattern |
...
| 11 | car: infotainment | Linux, Android Automotive, QNX | FULL BODY | LOW | AIR / C7 | HIGH | a laptop with a screen; nothing new |
...
| 16 | TV, set-top, console, camera | Android TV, Tizen, webOS, proprietary | FULL BODY | LOW | AIR / C7 | HIGH | a laptop with fewer actors; the store/update gate is an outside decision record |
...
| 23 | the seats of this estate (AI sessions) | Claude Code on Linux | ACTOR + a record (the board) | HIGH | now | HIGH | "how we dev = how gov-os is" (D08.9): the board is an append-only record with a fold; the transport law is a permission row |
