...
## ADDENDUM — 2026-09-17 — J8's DEFINITION SHARPENED: THE VAULT IS A HASH AND A COMPARE, NOT A STORE (the architect, on the owner's question; board :4435/:4437)

J8 as written above asked for exactly two things: the record proves a secret was used, and no actor reads the value. It never asked for the value's bytes to be kept anywhere. The module that built J8 (EP-19, src/kernel/vault.py) was cut from the content-addressed blob store "minus every read path" and carried that template's STORAGE with it: the secret's bytes written once to disk under their hash, with the honest cap of this paper's §7 saying the closure covered the system path and not the disk path, and that a cryptographic sealing of those bytes was campaign 4's work. A public scan of the pages named the bytes on 2026-09-17, and the owner asked why keep bytes no path reads. The answer at the definitive: a thing stored that cannot be derived and cannot be used is a template's residue, not a design. J8's vault IS a hash function and a compare; it keeps nothing; the record's hash and its MATCH / NO-MATCH decision are the whole of "verify, never reveal", and the §7 cap on the disk path is DISCHARGED by there being nothing on the disk to protect, not by cryptography. The system key sealed through the same vault (keys.seal_system_key) is covered by the same sentence. What the sharpening leaves open and names: the record's hash is an unsalted sha256 (secret_hash, the one hash home), so the hash of a low-entropy secret is guessable offline by a holder of the record; that is a question about the hash home, separate, and it is not closed here. The unit: MAINT-VAULT-HASH-ONLY (planning/exec), YELLOW, countersigned, built at the next cut.

AS-BUILT 2026-09-17 (the same day): MAINT-VAULT-HASH-ONLY closed at both hands (board :4462, :4463) — seal computes the hash and stores nothing; the three byte-freeze pins re-pointed; the seam census records the absence. One residue ruled to a green follow-on: the constructor still makes an empty vault directory, to be dropped (MAINT-VAULT-NO-HOME).
