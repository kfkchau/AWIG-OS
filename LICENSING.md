...
|---|---|---|---|
| **Core** | The operating-system engine: the governing layer (kernel, bridge, subsystems and founding) and the machine's own kernel in C and assembly under `code/src/body`, all shipped in this release under `code/src`, together with the runnable seed scripts `code/seed_demo.py` and `code/check.py`. | **GNU GPLv3** | Use it freely for anything; if you distribute a modified version, your modifications must be published under this same license. It stays open, in every hand, forever. |
...
Every source file in the Core under `code/src` begins with the SPDX header the renderer stamps:
