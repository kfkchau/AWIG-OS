...
Measured on 20 September 2026 on a Windows laptop under Debian in WSL 2 (Python 3.13.5), from a clean copy with nothing of ours installed: sixteen checks of sixteen; 170 test files run, 170 passed, 0 failed, 64 skipped naming what they need and 3 more that skip as they load; 2,779 test cases, none failed. The same again with the real locks installed (PyNaCl 1.6.2): the same figures. The build team ran the leak scan and the sixteen checks on its own render of the same commit and got the same. One timing test, `test_ep28c_w6.py`, can fail when the whole suite runs on a busy machine: run from a git checkout on the same laptop it failed in two whole runs and then passed five times of five alone. If you see it red, run it alone (`python3 tests/test_ep28c_w6.py`); if it is red alone, that is a finding and we want it.
