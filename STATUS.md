...
On 20 and 23 September two further rounds ran on this copy: a retest of every earlier finding, and a first assessment across fifty-four groups of the reviewer's own charter. The retest found the great majority of the earlier repairs hold. The new findings, repaired in the private repository on 23 and 24 September and shipping with the next release, in plain words: after an accepted recovery, one view of a file still served the damaged content while the record, its replay and a fresh mount served the recovered one; a kernel call that writes from a program's memory checked the start of the range and not its end, so a bad length could stop the guest (a local stop; no escape from the box was shown); a proof of inclusion accepted an empty path when the leaf equalled the root; two views handed back cached objects a caller could alter; and a boot whose sealed artifacts were not confirmed by its own record rows could act before refusing. One observation stays open: in the reviewer's environment the full kernel build's network device did not come up; on the reviewer's own source at our hand it did, and the request for their kept images is with them.
