...
diverging subtree instead:
    ...
    directory node       = canonical_hash over its children's        -- the relation OVER the children
                           sorted (childname, childhash) pairs
...
def _annotate(subnode):
    """Fold ONE structure node into `{kind, hash[, children]}`. A file leaf's hash is
    `canonical_hash` of its node; a directory's hash is `canonical_hash` over its children's SORTED
    (name, child-hash) pairs — the relation over the children, so a child that moves moves its
    ...
    children = {name: _annotate(child) for name, child in subnode["children"].items()}
    pairs = [[name, children[name]["hash"]] for name in sorted(children)]
    return {"kind": _DIR, "hash": canonical_hash(pairs), "children": children}


def merkle_tree(tree):
    ...
    two states (A2 — reported clean). One fold of the snapshot output; no record read."""
    return _annotate(_structure(tree))
...
def merkle_root(tree):
    ...
    same function (A1). The empty tree folds to a fixed, stable root (canonical_hash of no pairs)."""
    return merkle_tree(tree)["hash"]


def localize(pinned_tree, recomputed_tree):
    ...
    (and any node where a directory and a file trade places).
    ...
    out = []
    _walk(merkle_tree(pinned_tree), merkle_tree(recomputed_tree), "", out)
    return sorted(out)
