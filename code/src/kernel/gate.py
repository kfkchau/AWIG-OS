...
"""AWIG OS kernel — the gate (L1): the single registered write path.
...
# =====================================================================================
# THE SIGNED DOOR (EP-37; design/37 Q2/Q6/Q10) — per-account signature acceptance
# =====================================================================================
# For a key-bound account the gate accepts no act without a valid signature from a then-valid
# bound key (keys.bound_key, the EP-36 fold, read live as of the act's deciding time). The
# caller-asserted actor — carried by name since campaign 1 — retires at the cryptographic door,
# PER-ACCOUNT: binding a key IS the narrowing act, no dual-mode gate, no flag day (Q6, the EP-17
# transition lesson). An account with no bound key acts exactly as today under the openness grant.
#
# WIRED AT BIRTH. SIGN_LAW's PRESENCE in the live law is the door's gate (parallel to
# keys.KEY_LAW_BIND for the root-key leash): a founding that never declares it never enters the
# door. The create LANDED in production (COMPLETE-SIGNING-LAW, mover-2, founding 1.34.0; owner
# create-word board :2847, tier ruled ordinary/owner :2788): production founding now DECLARES
# SIGN_LAW, so the door is live — but production binds NO key (the first real key is the owner's own
# act, §8-g/W4), so the door engages NOBODY there and day one stays byte-identical for every unkeyed
# account (RW2). The retirement of caller-asserted acceptance for keyed accounts, licensed by the
# EP-17 strictly-stronger proof, is thus ENGAGED per-account (inert until a key binds), not held.
# Ordinary/owner-tier, NEVER constitutional
# (founding-only minting forbids new constitutional law in a running system; the acceptance
# mechanics are enforcement, not a legitimacy anchor — the anchor is the founding + root keys the
# door READS, not the door itself).
#
# TWO REFERENCES REFUSED (design/37 §5; EP-36's own note). THE QUIET ONE — sessions and tokens: a
# login minting a token, a "current signer" cache, an authenticated-session the gate consults, each
# storing identity STATUS between acts (the EP-15 trap in cryptographic dress). Refused by there
# being NO cache to consult — the account is resolved anew and the signature verified LIVE, every
# act. THE LOUD ONE — the blockchain/PKI pair: consensus where there is one sovereign store,
# certificate chains where verification bottoms out at the founding. Refused by there being no trust
...
def _signed_content(draft, deciding_time, seal):
    """The canonical bytes the invoker signs over (design/37 Q2): the act's CALLER-CONTROLLED
    content — action, object, target, payload — plus its deciding time and its seal where one
    exists. The gate-added fields (provenance, obligation_ref, the store envelope) are excluded on
    purpose: the invoker signs the act it INTENDED and the gate recomputes exactly that subset, so
    signer and verifier cannot drift. ONE hash via canonical.py — the estate's single serializer
    (T-CANONICAL-ONE-HASH; this EP adds none, design/37 §4).

    `payload` is normalised `or {}` to match the store's own normalisation (store._append_one:
    `ev.get("payload") or {}`), so a signature made over the draft (payload absent) still verifies
    over the appended record (payload {}) — the round-trip property (Q10). The deciding time is the
    invoker's CLAIM (None for a synchronous act, whose recording time = deciding time is bound by
    the store's countersignature, Q2's two-times split); the mint-time occurrence_time is NOT read
    here, so the sign-time and replay-time hashes agree."""
    return canonical_hash({"action": draft.get("action"), "object": draft.get("object"),
                           "target": draft.get("target"), "payload": draft.get("payload") or {},
                           "deciding_time": deciding_time, "seal": seal})


def make_invoker_sig(public_key, draft, deciding_time=None, seal=None):
    """The invoker's signature over an act, in the estate's signature model — possession-in-custody
    plus minted provenance, the shape of keys.countersign, NEVER a decrypt-and-sign (keys.py's own
    note; no cryptographic primitive is minted here — design/37 §9's stated cap). The mark CITES
    the signer's public key and BINDS the signed content. A real caller or a test produces it; the
    gate never signs — it only verifies (Q10)."""
    return {"by": public_key, "over": _signed_content(draft, deciding_time, seal)}


def verify_invoker_sig(mark, public_key, draft, deciding_time=None, seal=None):
    """A DERIVED check (Q10: recomputed on demand, re-signs nothing, keeps no state). True iff the
    mark cites EXACTLY `public_key` — the account's binding valid as of the act's deciding time —
    AND binds EXACTLY this act's signed content. A False is an honest no-match, and the ways it
    fails ARE the matrix's failing columns: absent -> not a dict; wrong key / superseded / bound
    only after the deciding time -> `by` mismatch or `public_key` is None; tampered -> `over`
    mismatch. A check that cannot fail is not a check (DIGEST-C2 §4).

    `Mapping`, not `dict`, so a signature verifies as DATA after replay too — the store deep-freezes
    a record to MappingProxyType, so the recorded signature read back off the record is a Mapping,
    not a plain dict (the round-trip property, Q10)."""
    return (isinstance(mark, Mapping) and public_key is not None
            and mark.get("by") == public_key
            and mark.get("over") == _signed_content(draft, deciding_time, seal))
...
class Gate:
    ...
    def _constitution_guard(self, actor, opname, draft):
        ...
        if draft_action == "DESIGNATE-SUCCESSOR":
            ...
            # THE SUCCESSION KEY HALF (EP-36 F5; design/37 Q8) — engages PER-ACCOUNT, never estate-wide
            # (archi :2855, EP-36-ENGAGE-AMEND). The check keys on the ACCOUNT being handed over, not on
            # whether the key-bind law is declared anywhere: a handover refuses a keyless successor ONLY
            # when the chain-end (ce) is ITSELF key-bound — keyed authority may never pass to a keyless
            # successor. A keyless account hands over on the succession fold alone, forever, until it
            # binds — exactly as the four settled EPs assert; declaring the law flag-days no account.
            # (An estate-wide key_law_live() gate here refused EVERY account's keyless handover the moment
            #  the shared founding declared the law — the flag day that contradicted this half's own intent
            #  and redded four settled CLOSED EPs; key_valid(ce) is that bug's account-scoped correction.)
            if self.views.key_valid(ce) and self.views.current_successor() is not None \
                    and not self.views.key_valid(self.views.current_successor()):
                self.refuse(actor, opname, "CONST-AUTHORITY-ANCHORED",
                            "the successor's key must be bound before the handover completes — no key, no "
                            "handover (the succession key half, CONST-AUTHORITY-ANCHORED in cryptographic form)")
        elif draft_action in (keys.KEY_ROTATE, keys.KEY_REVOKE):
            # THE ROOT KEY'S LEASH AT BIRTH (EP-36 F4; design/37 Q8 — CONST-AUTHORITY-ANCHORED in
            # cryptographic form). A rotate/revoke of the ROOT key — the key bound to the chain-end —
            # could orphan or move root authority, so it REFUSES without a successor key bound through
            # the recorded, human-only succession ceremony. A NON-root key rotates/revokes UNLEASHED
            # (the guard DISCRIMINATES, not blocks — RW1's near-miss: the same op WITH a successor
            # passes). Wire-at-birth and self-inerting: no world whose founding declares no key op can
            # mint such a draft, so this never fires there.
            if self.views.is_root_key_op(draft) and not self.views.successor_key_bound():
                self.refuse(actor, opname, keys.KEY_ANCHOR_RULE,
                            "the root key may not rotate or revoke without a successor key bound through the "
                            "human-only succession ceremony — root authority never orphans (the anchor in "
                            "cryptographic form; no root key retires without a bound successor)")
    ...
    def _signed_door(self, actor, name, draft, params):
        """THE SIGNED DOOR (EP-37 W1; design/37 Q2/Q6). In the acceptance region — an as-of-deciding
        check, the natural home for "is this act legitimately signed as of the moment it was
        decided". Returns the verified signature (recorded in provenance) or None; refuses through
        the gate's own `refuse` (recorded like every refusal), citing SIGN_LAW.

        INERT unless the founding declares SIGN_LAW: the whole pre-existing suite and production
        today never enter, so the zero-keys world is byte-identical (RW2). The flip is per-account —
        an account with a bound key MUST present a valid signature; an account without one continues
        under the openness grant unchanged (Q6). No signer STATE persists between acts: the invoking
        account is resolved FRESH and the signature verified LIVE against the EP-36 fold, every act
        (§8-h — the EP-15 trap in cryptographic dress, refused by there being no cache to consult).

        The key read is `bound_key(account)` — the binding valid as of the act's deciding time; for
        a synchronous act d = r, so the live fold IS the as-of-deciding answer (design/37 Q6, and the
        degenerate case that keeps the door byte-identical for the world it does not narrow). The
        as-of-deciding-POSITION read and its bound-after-the-deciding-time refusal ride the fold's
        own `as_of` (keys.bound_key, EP-36 A5); wiring an arrival's deciding-time key validity
        THROUGH the gate and the late-arriving-revocation sweep is EP-38's two-frames arithmetic,
        fenced out of this EP (RW4)."""
        if SIGN_LAW not in self.views.active_rules():
            return None                                   # door not wired in this founding -> inert
        account = self.views.resolve_asserted_by(actor)   # the invoking account, resolved anew (no session)
        from . import reconcile as _reconcile
        seal = draft.get("seal")
        carries = _reconcile.carries_deciding_time(draft)
        # THE INVOKER'S CLAIM is the occurrence_time STRING it signed over (design/37 Q2 — the signed
        # content binds the caller's own claimed deciding time, and canonical.py serialises the string,
        # never a parsed datetime); the comparable INSTANT is the parsed moment the as-of-d key fold
        # needs (`in_force_at`). Both are None for a synchronous act (no distinct claim at acceptance),
        # so the door is byte-identical to EP-37 there — the arrival path below is the only new one.
        claimed = draft.get("occurrence_time") if carries else None
        instant = _reconcile.deciding_time(draft) if carries else None
        # THE KEY IS READ AS OF THE ACT'S OWN DECIDING TIME (EP-38 W2, the arrival case wired out of
        # EP-37 as RW4). For a SYNCHRONOUS act d = r, so the live fold IS the as-of-deciding answer
        # (instant is None and the branch below is the live read). For an ARRIVAL that decided
        # elsewhere and travelled, the live fold is the WRONG fold: a key valid at the deciding moment
        # but superseded before the arrival must STILL verify (a then-valid signature is evidence, the
        # legitimacy frame), and a key superseded before the deciding moment must FAIL however live it
        # is now. So the binding is folded AS OF the instant over a counterfactual world whose law_asof
        # is that instant and whose `_selects` folds key records by deciding time. In a re-adjudication
        # the views ARE already that world (its store is a WorldStore), so reading it directly is the
        # identical answer without nesting a world. No status is stored between acts (§8-h): the fold
        # is derived live, every act.
        if instant is None or isinstance(self.store, _reconcile.WorldStore):
            bound = self.views.bound_key(account)         # synchronous, or already an as-of-d world
        else:
            bound = _reconcile.world_for(self.store, draft, law_asof=instant).bound_key(account)  # arrival: as of d
        mark = params.get(INVOKER_SIG) if params else None
        presented = mark if isinstance(mark, Mapping) else None   # a real signature, not SIG_PENDING/None
        if bound is not None:
            if presented is not None and verify_invoker_sig(presented, bound, draft, claimed, seal):
                return presented                          # a valid signature -> accept
            self.refuse(actor, name, SIGN_LAW,
                        f"{account} is key-bound: the gate accepts no act without a valid signature "
                        f"from its bound key, verified live as of the act's deciding time", draft=draft)
        if presented is not None:
            # no valid binding as of the deciding time, yet a signature is presented: it cites a key
            # that is wrong, superseded, or bound only AFTER this act's deciding time. A signature
            # over an invalid binding is a false claim, not openness (the matrix's fail columns).
            self.refuse(actor, name, SIGN_LAW,
                        f"{account} presented a signature citing a key that is not its valid binding "
                        f"as of the act's deciding time", draft=draft)
        return None                                       # unkeyed and unsigned -> openness, act as today
    ...
    def _decide(self, name, actor, params=None):
        ...
        if isinstance(result, dict) and "seq" not in result:
            ...
            # ---- THE CEILING (EP-38 W1; design/35 §6 closing direction, design/37 Q7) ----
            # The claimed deciding time is now bracketed on BOTH sides: the seal bounds it below (a
            # claim cannot predate the world its pack pins), the countersigned arrival bounds it above
            # (a claim cannot postdate the moment the store witnessed writing it). Both cite the
            # existing two-times law; no founding law is minted (§8-i). Inert for a synchronous act,
            # whose deciding time is minted at append, so the existing world is byte-identical.
            _reconcile.ceiling_check(self, self.store, actor, name, result)
            ...
            # ---- THE SIGNED DOOR (EP-37 W1; design/37 Q2/Q6) ----
            # For a key-bound account the gate accepts no act without a valid signature from a
            # then-valid bound key; verified live against the EP-36 fold, no signer state kept.
            # SIGN_LAW is declared in production now (mover-2, 1.34.0), so the door is live; it is
            # INERT for every account with no bound key (production binds none), so day one is
            # byte-identical (RW2).
            verified_sig = self._signed_door(actor, name, result, params)
            ...
            # THE INVOKING ACCOUNT'S SIGNATURE RIDES IN PROVENANCE (design/37 Q2), never a new
            # envelope field (§8-i): recorded where the door verified one, replayed as data (Q10).
            if "provenance" not in result:
                prov = {"asserted_by": self.views.resolve_asserted_by(actor),
    ...
    def _maybe_sweep(self, record):
        ...
        from . import reconcile as _reconcile
        # A late-arriving LAW reconciles by the sweep; so does a late-deciding key REVOCATION (EP-38
        # W3, design/37 Q7: "revocation is law-family for the sweep"). A revoke is DECISION-class and
        # carries no rule_id, so `is_law_family` misses it — `triggers_key_sweep` recognises it by its
        # class tag. An act accepted on a key the revoke supersedes AS OF the act's own deciding time
        # is re-adjudicated by the SAME §4b machinery, nothing new invented; the world's deciding-time
        # key fold does the reach and the re-run signed door does the overturn.
        if not (_reconcile.is_law_family(record) or _reconcile.triggers_key_sweep(record)):
            return
