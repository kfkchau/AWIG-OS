...
"""AWIG OS kernel — operations as definition records (design/27 §3, build step three).
...
# design/36 ADDENDUM I.2). Which members NAME A CALLER PARAMETER, and the envelope FIELD each
# one supplies, is DECLARED here rather than inferred from a `_param` suffix (EP-33A item 20).
# The governance content — which member routes a caller parameter — used to live in the spelling
# of the member's name: true today, tested, and wrong the day a member is spelled against the
# convention, which is the code-verb-list shape one step subtler (the truth living somewhere
# other than where it is declared). A declaration cannot be spelled wrong, so a member is a
# param-router because it is DECLARED one, never because of how it happens to be spelled. The two
# members absent from the map name no caller parameter at all — `content_form`, a literal string
# on the record, and `stamp_actor`, a list of payload fields overwritten with the acting actor —
# so there is no field for them to route one into.
ROUTER_FIELDS = {"target_param": "target",
                 "evidence_param": "evidence_summary",
...
#
# MEMBERSHIP IS READ FROM THE DECLARED MAP, not from the `_param` spelling. The family's order is
# preserved by walking `ENVELOPE_ROUTERS` and keeping exactly the members `ROUTER_FIELDS`
# declares — so a member spelled against the convention is still found, and one spelled with the
# convention but absent from the map is still excluded.
PARAM_ROUTERS = tuple(r for r in ENVELOPE_ROUTERS if r in ROUTER_FIELDS)
...
# THE CONTENT-PLACEMENT FAMILY, MEMBERS 1-2 (design/46 §2; EP-41A). Two founding LAWS and one
# definition-shape clause, all read from the record plane, none a hard-coded flag.
#
# MEMBER 1 — THE PLACEMENT LAW. Declared content-interiors live outside the row as
# content-addressed blobs (the built `content_params` mint below, opdefs `run`: bytes to
# `gate.blobs.put`, the row keeps the HASH); the routing criterion is the op's OWN declaration,
# and NO SIZE NUMBER exists or ever enters (design/46 §2.1: a number is friction pretending to be
# coverage). This constant is the rule_id the founding CREATE-RULE declares when MEMBER 1 is
# lawed; its PRESENCE is what a placement-law reader would read, never a coded boolean.
PLACEMENT_LAW = "PLACEMENT-LAW"
#
# MEMBER 2 — THE VOCABULARY DOOR. No op definition enters unless EVERY payload field is
# classified STRUCTURAL or CONTENT, so unclassified content cannot live inline in any row by
# construction (design/46 §2.2). CONTENT is the already-built declaration (`content_params` /
# `secret_params` / a `secret_verify` candidate — the value never lands in the row, only a hash);
# STRUCTURAL is the new `structural_params` declaration below (fields the author AFFIRMS are
# governance fields, safe inline). The completeness clause runs inside `validate_definition_shape`
# at all THREE doors, but ENGAGES ONLY WHERE THIS LAW IS LIVE (`VOCAB_DOOR_LAW` present in the
# live founding — the `key_law_live` wire-at-birth idiom, keys.py): a world whose founding does
# not declare it classifies nothing and founds exactly as before, which is what keeps the HELD
# founding-move (the create + the census sweep, MOVER 4, owner-gated) from moving a byte until the
# mover lands. Read from the record plane (active_rules at the runtime doors; the pack's own rule
# records at the founding door), never a hard-coded flag.
VOCAB_DOOR_LAW = "VOCAB-DOOR-LAW"
#
# The definition key the author uses to CLASSIFY a payload field STRUCTURAL: a list of param names
# affirmed to carry no reachable content, so they are safe to live inline in the record. The
# mirror of `content_params` (which classifies CONTENT); together they must cover every declared
# param, or the vocabulary door refuses the definition (when the law is live). The one-time census
...
def validate_definition_shape(gate, doorname, actor, name, d, peers=None, classify_law_live=False):
    """THE DEFINITION-SHAPE VOCABULARY, IN ONE PLACE, RUN AT ALL THREE DOORS (EP-27B ADDENDUM 2
    W6c; design/36 ADDENDUM I.3).

    `classify_law_live` (EP-41A, design/46 member 2) is the ONE law-plane input this door reads:
    is the vocabulary-door law (`VOCAB_DOOR_LAW`) declared in the live founding? It arrives as an
    argument rather than being read here because the answer's SOURCE differs per door — the live
    `active_rules` at the two runtime doors, the pack's own rule records at the founding door — and
    the shared function must not couple to either. False (the default) is the pre-mover world:
    every clause below runs unchanged and the classification clause is inert, so a world founded
    before the content-placement create classifies nothing and founds exactly as before (the
    `key_law_live` wire-at-birth idiom). This is what holds MOVER 4 until the owner's word.
...
def _content_home_params(d):
    """THE PARAMS ALREADY CLASSIFIED CONTENT — the value never lands inline in the row, only a
    hash (EP-41A member 1's built mechanism). Read from the definition's own declarations, in one
    place, so the vocabulary door (member 2) and the census sweep never diverge on what "content"
    means: `content_params` (blob store, opdefs `run`), `secret_params` (vault, sealed), and a
    `secret_verify` CANDIDATE (crosses once, compared by hash, never recorded). NO SIZE NUMBER is
    read here or anywhere on this path — placement and classification are DECLARATION-driven."""
    home = set((d.get("content_params") or {}).keys())
    home |= set((d.get("secret_params") or {}).keys())
    sv = d.get("secret_verify")
    if isinstance(sv, dict) and sv.get("candidate_param"):
        home.add(sv["candidate_param"])
    return home


def structural_field_classification(d):
    """THE ONE-TIME CENSUS SWEEP, FOR ONE DEFINITION (EP-41A member 2; design/46 §2). The
    STRUCTURAL classification of an existing op is every declared param that is not already
    content-classified — the fields that carry no reachable content and so are safe to live inline.
    Pure and deterministic over the definition's own `params`; the sweep that rides the MOVER-4
    create applies this to the DRIVEN op census (`install.op_definitions()`, never a named list,
    §A57), and the acceptance battery applies it in a test founding world where the law is live.
    Content-classed params are EXCLUDED — their classification is `content_params`/`secret_params`,
    not this list — so the two classes stay disjoint by construction."""
    home = _content_home_params(d)
    return [p for p in (d.get("params") or {}) if p not in home]


def _require_field_classification(gate, doorname, actor, d):
    """THE VOCABULARY DOOR (EP-41A member 2; design/46 §2.2). No op definition enters unless EVERY
    payload field is classified STRUCTURAL or CONTENT — so unclassified content cannot exist inline
    in any row, and the catch is at the RULE layer (this door, change slow and owner-gated), not at
    the act layer. A param is CONTENT-classified by `content_params`/`secret_params`/a
    `secret_verify` candidate (`_content_home_params`); STRUCTURAL-classified by the
    `structural_params` list (author-affirmed governance fields, safe inline). A declared param in
    NEITHER class is UNCLASSIFIED — refused as a malformed definition (AR-2, the shape-rule its
    sibling clauses cite), the message naming the vocabulary door — because the author has not said
    whether it carries reachable content, and silence is exactly the collision this door closes
    (design/46 §2: content living inline where custody could never be conserved for it).

    THE DOOR DISCRIMINATES, IT DOES NOT OVER-BLOCK: a fully-classified definition (every param in
    one class) passes, so R1 drives both ways — an unclassified field refuses and a well-formed one
    admits. Completeness only: whether a field DECLARED structural actually carries content is the
    named residual (design/46 §4, misclassification — auditable by census, an EP's standing view),
    not this door's question; and NO SIZE NUMBER is consulted anywhere here (design/46 §2.1).

    Engaged only when the caller passed `classify_law_live` — the wire-at-birth gate that keeps the
    HELD founding-move (create + sweep, MOVER 4) inert until the owner's word."""
    home = _content_home_params(d)
    structural = set(d.get(STRUCTURAL_PARAMS) or [])
    classified = home | structural
    for param in (d.get("params") or {}):
        if param not in classified:
            gate.refuse(actor, doorname, "AR-2",
                        f'payload field {param!r} is unclassified — every op definition\'s payload '
                        "field must be classified STRUCTURAL (declared in structural_params, safe "
                        "inline) or CONTENT (declared in content_params/secret_params, blob-homed by "
                        "hash) before it enters the founding, so unclassified content cannot live "
                        "inline in any row where custody could never be conserved for it "
                        "(the vocabulary door — design/46 member 2)")
...
# The fields a `ceiling` row must declare so its SUMMED value carries its population and its basis
# (charter §A51). `aggregate_field` is the BASIS — the field summed. The rest declare the POPULATION
# the fold runs over: `aggregate_action` (which records contribute), `holder_field` + `holder_param`
# (scoped to this holder), `key_param` (keyed, so a re-grant overwrites rather than double-counts),
# `removal_action` (records a later act removes from the set), `policy_key_prefix` + `amount_param`
# (the ceiling read and the requested delta the sum is compared against). Every one of these is a
# field `_ceiling_check` already reads by `c[...]`, so a row missing any would KeyError at the act;
# refusing it at the door turns that latent crash into a stated §A51 refusal.
_CEILING_A51_FIELDS = ("policy_key_prefix", "holder_param", "aggregate_action", "holder_field",
                       "key_param", "aggregate_field", "removal_action", "amount_param")


def _require_wellformed_ceiling(gate, opname, actor, d):
    """A `ceiling` CHECK SUMS A NUMERIC FIELD OF PRIOR RECORDS, AND A SUMMABLE VALUE THAT DOES NOT
    CARRY ITS POPULATION AND ITS BASIS IS REFUSED (charter §A51; MEM-LAW-BUDGET's own wording).
    Definition time, all three doors — the CREATE-OP door, the AMEND-OP door, and the founding
    installer through `_FoundingDoor`.

    This is the ceiling kind COLLIDING with §A51 (the comment at REQ_BINDING) resolved rather than
    left standing: the kind stays the existing summing kind (no seventeenth operator is minted), and
    the leash requires its declaration to NAME the population it folds and the basis it reads. A
    ceiling row that declares `aggregate_field` (it sums) while omitting the population declaration
    is a BARE SUMMABLE — a scalar with no provenance — and the door refuses it, citing §A51. A
    create that cannot be refused is not a gate: this leash is what lets a bare summable be refused
    and a well-formed one admitted, which R1 drives both ways."""
    for c in (d.get("checks") or []):
        if c.get("check") != "ceiling":
            continue
        for field in _CEILING_A51_FIELDS:
            v = c.get(field)
            if not isinstance(v, str) or not v:
                gate.refuse(actor, opname, "AR-2",
                            'a "ceiling" check sums a numeric field of prior records but declares '
                            "no %r — charter §A51 forbids a summable value that does not carry its "
                            "POPULATION and its BASIS, so a bare summable (one that sums without "
                            "naming the records it folds and the field it reads) is refused at the "
                            "definition door" % field)
...
def _ceiling_check(gate, store, views, actor, opname, c, params):
    ...
    aggregate is an ORDERED keyed fold over the check's OWN FAMILY in seq order — the
    records carrying `aggregate_action` or `removal_action`, read through a held config-keyed
    projection (K12; MAINT-K12-CEILING-FOLD) — NOT two by-action scans and NOT the whole
    record — so grant/evict/re-grant interleave correctly:
    ...
    # K12 (design/36 ADDENDUM S; MAINT-K12-CEILING-FOLD, board :2727 A2): the fold's answer
    # depends only on this check's family — the records carrying `aggregate_action` or
    # `removal_action` — so it iterates THAT family (held, seq-ordered, config-keyed) rather than
    # the whole record. The family is derived from THIS check's own declared config, so no second
    # recognition rule and no subsystem import; the fold body below is byte-unchanged (one
    # implementation, two sources — store.py:268), which is exactly what the differential tests.
    family = store.action_set_projection({c["aggregate_action"], c["removal_action"]})
    agg = {}
    for e in family.all():
    ...
    def run(actor, params):
        ...
        def _dispatch_check(gate, c, p_in):
                if c["check"] == "require_prior":
                    ...
                    # IDENTITY IS THE ENGINE'S FACT, NOT THE CALLER'S CLAIM (EP-30-C4W; design/41
                    # Q1 actor-slot-is-identity, Q2 verification-order-is-law, both owner-ruled
                    # 2026-08-22; directing ruling archi board :2610 direction B). If the subject
                    # is a field THIS op stamps with the acting actor (`stamp_actor`), the stamp
                    # loop below overwrites it with `actor` in the record (`payload[f] = actor`),
                    # never back into `p_in` — so the caller value read above is the PRE-STAMP
                    # claim, forgeable by construction, and the check must instead read that same
                    # `actor`, the engine's fact. This is an OVERRIDE on the caller-read, not a
                    # replacement of it: the base read stays `p_in.get(c["param"])` for every
                    # caller-choosable subject (EP-30-K1's minimality gate reads exactly that),
                    # and only a stamped subject is redirected. Membership is read from THIS op's
                    # OWN `stamp_actor` declaration — the exact list the stamp loop reads — and
                    # NEVER from a field-name string match, which would be a new latent kind and
                    # STOPS (design/28 §5; the plan's §8.3). NO check kind and NO vocabulary member
                    # is added; `OP_CHECKS` and §5's table are untouched (:2610 B). It is a no-op
                    # for every shipped check — none resolves a subject that is a stamped field of
                    # its own op (EP-30-C4W A2/A4) — so no shipped verdict moves; the seam is
                    # closed BEFORE a check is ever pointed at a stamped field, not after.
                    if c["param"] in (d.get("stamp_actor") or ()):
                        want = actor
...
def register_opdef_ops(gate, store, views):
    ...
    def create_op(actor, params):
        ...
        validate_definition_shape(gate, "CREATE-OP", actor, name, d,
                                  ...
                                  classify_law_live=VOCAB_DOOR_LAW in views.active_rules())
