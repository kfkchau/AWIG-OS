...
"""AWIG OS kernel — the canonical serializer (EP-23 W3; design/34 §5).
...
# ── ENGINE SURFACE — the estate's SINGLE governed canonical serializer (design/37 §4; EP-34-BUILD) ──
# This module is THE floor. design/37 §4: "the canonical serializer is engine surface (S-plane) …
# one serializer, one fuzz battery, two consumers; a second serializer anywhere is a defect by
# construction." Its one-state-one-hash exactness is proven by the T-CANONICAL-ONE-HASH fuzz battery,
# and its uniqueness is ENFORCED (not merely asserted by use) by the one-serializer guard
# T-CANONICAL-ONE-SERIALIZER — both in tests/test_ep34.py. The guard enumerates every site in src/
# that hashes a canonically-serialized structure and reds unless the set is exactly this module plus
# a CLOSED named exemption.
#
# THE CLOSED EXEMPTION — the DUAL-AUDIT DIGEST family {protection.py:_digest, views.py:digest_of}.
# Those two are NOT this serializer, and the distinction is the whole reason they are exempt rather
# than unified (RULED U-B, architect board :2742):
#   * the canonical serializer answers "what are this state's canonical bytes" — seal/chain IDENTITY,
#     the type-tagged encoding below (NOT JSON: the ambient JSON dump is the named wrong reference,
#     design/37 §5 — it has several spellings per value and imports platform variance);
#   * the dual-audit digest answers "does this audit mirror still match its referent" — a 16-char
#     sha256(json.dumps(sort_keys)) cross-check whose value is FROZEN BY HISTORY, persisted inside
#     never-rewritten records (protection.py:108/123 write it, :132 verifies fresh-vs-STORED).
# The list is CLOSED: a THIRD governed hash site in src is a guard RED, never a silent list edit. The
# two exempted outputs are as frozen as this module's (a changed dual-audit digest orphans every
# sealed mirror record). The true unify (dual-audit digests computed through canonical form) is
# re-homed to EP-35's era boundary — new records only, the past never rewritten.
ENGINE_SURFACE = "canonical-serializer"
