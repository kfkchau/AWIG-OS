...
"""AWIG OS kernel — the two times of authority and the reconciliation sweep (EP-26;
...
from .gate import Gate, INVOKER_SIG
...
def is_key_legitimacy(record):
    """A KEY-LIFECYCLE record — bind, rotate or revoke (keys.KEY_KINDS). These carry KEY VALIDITY,
    which lives in the LEGITIMACY frame exactly as law does: judged AS OF the deciding time, never
    at arrival order (design/37 Q7 — "key validity is judged in the legitimacy frame"; the K7 split
    of design/34 §2, applied to keys). A key-revoke/rotate is DECISION-class and carries no rule_id,
    so `is_law_family` misses it; its supersession nonetheless reaches back like a late-arriving law,
    which is why the world's selection folds it by deciding time (`_selects`) rather than by seq.

    Recognised by the payload's class tag (`kind` in keys.KEY_KINDS), never by an action-name list —
    the same self-declaration `keys._key_records` uses, so the fold reads a key act however its op was
    spelled."""
    return _payload(record).get("kind") in keys.KEY_KINDS


def triggers_key_sweep(record):
    """Does this record TRIGGER a reconciliation sweep as a late-arriving legitimacy fact? The order
    scopes the key-sweep to REVOCATION (design/37 Q7; EP-38.md W3: "a key revocation whose deciding
    time precedes already-accepted acts is exactly design/36 §4b's late-arriving law"). A revoke
    supersedes a binding to none; an act accepted on that key, decided strictly after the revoke's
    deciding moment, was accepted against a key the revoke had already retired — so its acceptance is
    re-adjudicated by the EXISTING sweep, nothing new built. A key-BIND or key-ROTATE never triggers a
    sweep: those are handled at the ordinary door (validity-as-of-deciding) and, within an
    already-running sweep, by the world's own deciding-time key fold."""
    return _payload(record).get("kind") == keys.KEY_REVOKE
...
class WorldStore:
    ...
    def __init__(self, store, law_asof, arrival_seq, excluded=(), head_seq=None):
        ...
        self._projections = {}
        # ADDITIVE (KEY-PROJECTION, board :2910): the per-account key-projection cache, sibling of
        # `_projections` above and of EventStore's own `_key_projections`. Touches nothing else —
        # `_selects` and the read surface are unchanged. See `key_projection` below.
    ...
    def _selects(self, e):
        ...
        if seq > self._head or seq in self._excluded:
            return False
        # LAW and KEY-LIFECYCLE records are the LEGITIMACY frame: in this world iff their deciding
        # time is strictly before the act's (`in_force_at`) — so a late-arriving law OR a
        # late-deciding key supersession the act never saw at its own recording time is nonetheless
        # present here. This is the whole mechanism of reach, now carrying keys (EP-38 W2/W3: the
        # K7 split applied to keys). Reading `bound_key` over a world whose law_asof is d gives the
        # key VALID AS OF d — the arrival's own deciding-time key fold, one selection rule for both.
        if is_law_family(e) or is_key_legitimacy(e):
            return in_force_at(e, self._law_asof)
        # every other record: authority live at effect time — in this world iff it precedes the act.
    ...
    def key_projection(self, acct):
        """A per-account KEY projection over THIS world's events (KEY-PROJECTION, board :2910,
        Option A). Mirrors `record_projection` above: built on first use, cached, resolves through
        THIS world. Because this world's `events` are ALREADY folded by deciding time (`_selects`,
        which puts a key-lifecycle record in the world iff its deciding time is `in_force_at` the
        act's), a KeyProjection over it selects the account's key records valid AS OF the act's
        deciding moment — so `keys.bound_key` routed through it answers the as-of-past frame
        IDENTICALLY to the fold reading this world directly (binding 1, one read path both frames).

        ADDITIVE: it does not touch `_selects` or the existing read surface; it is the key-family
        sibling of `record_projection`, read by the signed door's arrival re-adjudication
        (`admits` -> `DryGate._signed_door` -> views.bound_key over this world). The cache-key
        parameter is `acct`, not `account`, for the same T-NO-STORED-TABLE reason as the store's
        `key_projection` — no subject-of-power subscript."""
        from .store import KeyProjection
...
def admits(gate, world_views, record, opname=None):
    ...
    try:
        ...
        # EP-38 W3: THE SIGNED DOOR RE-RUNS DURING RE-ADJUDICATION, so an act accepted on a key that
        # a late-deciding revocation supersedes AS OF the act's own deciding time is overturned. The
        # world (world_views) already folds keys by deciding time (`_selects`), so its `bound_key`
        # answers as of d — a key revoked before d reads None and the door refuses (a signature over
        # an invalid binding is a false claim, exactly the door's live logic, EP-37). The recorded
        # signature rides in provenance (Q2); it is re-presented as the door's param and verified
        # LIVE — no signer state is stored anywhere between acts (§8-h, the EP-15 trap refused). The
        # door is INERT unless the world declares SIGN_LAW, so every pre-signing sweep is unchanged.
        sig = (record.get("provenance") or {}).get(INVOKER_SIG)
        dry._signed_door(actor, name, draft, {INVOKER_SIG: sig} if sig is not None else {})
...
# ---- the ceiling (W1; design/35 §6 closing direction; design/37 Q7) --------------------------

def _arrival_instant():
    """The moment the store is witnessing this arrival — the wall clock, read exactly as
    `store._append_one` mints `record_time`. Acceptance runs microseconds before the append, so
    this is a faithful pre-append reading of the countersigned arrival the ceiling bounds against.
    Isolated in one place so a test can hold the clock still if it ever needs to."""
    import datetime
    return datetime.datetime.now(datetime.timezone.utc)


def arrival_ceiling(record, arrival):
    """A CLAIMED DECIDING TIME CANNOT POSTDATE THE STORE'S COUNTERSIGNED ARRIVAL.

    The seal bounds how far a claim runs BACKWARD (`seal_bound`); this bounds how far it runs
    FORWARD. The two-times seal-bound law names this direction in its own founding text — "the other
    direction, a claimed time running later than the true one, stays open until record signing closes
    it" (TWO-TIMES-SEAL-BOUND) — and record signing is this campaign. A decision cannot honestly be
    claimed at a moment LATER than the one the store witnessed writing it: the store countersigns the
    recording fact (seq, record_time) at append, so `occurrence_time` > that arrival is a claim to
    have decided the act after it was already recorded.

    Existential and honest like the floor: it refuses only a claim STRICTLY after the arrival —
    within the bracket the deciding time remains the signer's assertion, stated not hidden (design/37
    §9). Returns (ok, reason)."""
    claimed = deciding_time(record)
    if claimed is None or arrival is None:
        return True, None
    if claimed > arrival:
        return False, (
            f"claims deciding time {record.get('occurrence_time')} later than the store's "
            f"countersigned arrival ({arrival.isoformat()}) — a decision cannot be claimed at a "
            f"moment after the one the store witnessed recording it")
    return True, None


def ceiling_check(gate, store, actor, opname, draft, arrival=None):
    """THE CEILING, AT THE CHOKEPOINT — the acceptance step's forward bound, paired with the floor.

    Fires on any arrival that carries a distinct deciding time (no seal needed — the ceiling is about
    the deciding time versus the arrival, independent of any sealed world). Refuses citing the
    two-times acceptance law (TWO-TIMES-ACCEPT), the SAME founding law the floor's sibling extends:
    no new founding law or kind is minted (§8-i), the ceiling is the existing two-times law's other
    bound finally wired. A synchronous act carries no distinct deciding time at acceptance (its
    occurrence_time is minted at append), so the step never enters for it — the existing world is
    byte-identical."""
    if not carries_deciding_time(draft):
        return
    ok, reason = arrival_ceiling(draft, arrival if arrival is not None else _arrival_instant())
    if not ok:
        gate.refuse(actor, opname, ACCEPT_RULE, reason, draft=draft)
