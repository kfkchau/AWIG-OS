...
class TestProductionAttests(unittest.TestCase):
    ...
    def test_compose_registers_the_op_and_attests_live_at_boot(self):
        ...
        self.assertEqual(len(ATTESTED_MEMBERS), 88)                 # 88 since C7 P3b-6a's +1 net.c (the virtio-net driver, archi :4250); 87 since C7 P3b-4b's +2 serve files (serve.c/serve.h, archi :4089); 85 since C7 P3b-4a's enclosure add (+6 src/body files enclosure.S/enclosure.c/enclosure.h/sha256.c/worker.S/worker.ld, archi :4085); 79 since C7 P3b-3's clock/interrupt/entropy add (+6 src/body files clkcheck.c/clock.c/clock.h/entropy.c/idt.c/isr.S, archi :4049); 73 since C7 P3b-2's disk add (+5 src/body files ata.c/bodyfs.c/disk.h/diskcheck.c/mkdisk.py, archi :4019); 68 since C7 P3b-1's first-merged-body add (+11 src/body files: the memory C + boot.S + linker.ld + the generated rows-digest header + the derive-from-rows generator .py + build.sh; §9 mechanism 1 ONE ATTESTED LIST, archi :4009). 57 since C7 P7a's interpreter-hosting layer joined (design/54 §7 P7; +2: hosting/__init__.py, hosting/interpreter_host.py — our own signed code that supplies CPython's host surface). the enumerated S-plane set (55 since C7 P2-SEAM-AS-A-CONTRACT's bridge/host_seam.py add — the effect-seam performer interface, design/54 §7 P2, §5 count-pin companion driven at dispatch; 54 since P12-CONSTITUTION-SURVIVES-BUILD's bridge/reconstruct.py add — C6 P12, B6/I15, the §5 count-pin companion driven at the fence amendment (mgr :3631); 53 since EP-52-BUILD's subsystems/filter.py add — FIREWALL-IN-THE-RECORD, C5 P7, the founding mover, §5 count-pin companion driven at dispatch; 52 since EP-49A-BUILD's kernel/border.py add — THE BORDER: THE DOOR, the campaign-5 crux, §5 fence "+ its ATTESTED_MEMBERS line and count-pin companion", DRIVEN at dispatch not carried; 51 since EP-48-BUILD's subsystems/sockets.py add — SOCKET-GRANTS, the campaign-5 founding mover, §5 name-and-count sweep + :3255(b) by-name widen, KEY-MATERIAL-REAL's signer.py already in at 50; 50 since KEY-MATERIAL-REAL's kernel/signer.py add — archi :3249, the by-name count-pin widen doubled from crypto.py; 49 at kernel/crypto.py, the :3031/:3041 double-stop companion; 48 at EP-46-BUILD's bridge/seal.py add, board :3031; 47 at EP-43-AUTO's bridge/auto_recover.py add; 46 at EP-45's bridge/merkle.py add, :3045)
        self.assertEqual(len(rec["payload"]["members"]), 88)       # 88 since C7 P3b-6a's +1 net.c (the virtio-net driver, archi :4250); 87 since C7 P3b-4b's +2 serve files (the boot-attestation's recorded member count, the :259 twin); 85 since C7 P3b-4a's enclosure add (+6 src/body files enclosure.S/enclosure.c/enclosure.h/sha256.c/worker.S/worker.ld, archi :4085); 79 since C7 P3b-3's clock/interrupt/entropy add (+6 src/body files, archi :4049 — the boot-attestation's recorded member count, the :259 twin); 73 since C7 P3b-2's disk add (+5 src/body files, archi :4019 — the boot-attestation's recorded member count, the :259 twin); 68 since C7 P3b-1's first-merged-body add (+11 src/body files — the boot-attestation's recorded member count, the :259 count-pin's twin; §9 archi :4009). 55->57 since C7 P7a's interpreter-hosting layer add (+2: hosting/__init__.py, hosting/interpreter_host.py — the boot-attestation's recorded member count, the :259 count-pin's twin); 55 since C7 P2-SEAM-AS-A-CONTRACT's bridge/host_seam.py add (the boot-attestation's recorded member count, the :259 count-pin's twin); 54 since P12-CONSTITUTION-SURVIVES-BUILD's bridge/reconstruct.py add (C6 P12, the §5 count-pin companion driven at the fence amendment, mgr :3631 — the boot-attestation's recorded member count, the :259 count-pin's twin); 53 since EP-52-BUILD's subsystems/filter.py add (§5 count-pin companion, driven at dispatch); 52 since EP-49A-BUILD's kernel/border.py add (the boot-attestation's recorded member count, the :259 count-pin's twin; driven at dispatch)
